...
- Phase 1 is partially complete: typed env config, request-scoped TLS behavior, centralized dotenv loading, tested CORS whitelist policy, `PVEClient`, `GuacamoleApiClient`, `OpenAIClientFactory`, `OpenAICompatibleChatClient`, and `OpenCodeRunner` are in place. Auth token user ID validation now prevents malformed token payload IDs from reaching Mongoose lookups. Auth registration input/conflict rules are extracted/tested and registration duplicate checks use one batched identity lookup. Auth login workflow, wrong-attempt lockout, unverified-email resend, and token response behavior are extracted/tested. User profile/password/avatar and user courses/CRP/SuperAdmin lookup workflow services are extracted/tested. More domain-specific service splits remain.
