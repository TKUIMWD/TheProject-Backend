...
- Current service facades no longer accept Express `Request` directly; controllers now own token/header/body/query/params extraction and pass DTO inputs into services.
...
- Phase 2 service-boundary cleanup is complete for the current service layer: `TemplateManageService`, `SuperAdminCRPService`, `VMService`, `PVEService`, `TemplateService`, `AIBoxBuildService`, `AIChatService`, `UserService`, `VMManageService`, `GuacamoleService`, `CourseService`, and `VMBoxService` now accept DTO/actor inputs instead of raw Express `Request`; `src/service` has no Express `Request` or direct auth-helper imports.

### 2026-05-26 Remaining Service Facade Request Boundary Slice

- Moved token/header/body/query/params extraction for Template Manage, CRP, VM read, PVE, Template, AI Box Build, AI Chat, User, VM Manage, Guacamole, Course, and VM Box facades into controllers.
- Kept existing service-to-workflow adapter modules as the business boundary; services now forward DTO inputs only.
- Preserved public route behavior for Course public list and VM Box public list/writeups while protected routes still validate in controllers.
- Verification:
  - `npm run typecheck`
  - targeted facade-boundary tests: `10` files / `54` tests, `12` files / `58` tests, `8` files / `47` tests, and `22` files / `102` tests
  - `npm test` (`187` files, `924` tests)
  - `npm run build`
  - `npm audit --audit-level=moderate` (`0` vulnerabilities)
  - merge-conflict marker scan, backend `console.*` scan, `src/modules` reverse-import scan, `src/service` Request/auth-helper scan, and `git diff --check`
...
### 2026-05-26 VM Operation Facade Request Boundary Slice

- Added `tests/vm-operate-service.test.ts`.
- Moved VM operation token/body extraction out of `VMOperateService` and into `VMOperateController`.
- `VMOperateService` now exposes DTO-style methods for boot/shutdown/poweroff/reboot/reset and delegates operation execution through an injectable port.
- Controller response shapes remain unchanged; VM operation route methods still send the same `resp` payload with the returned status code.
- `VMOperateService.ts` is now about `65` lines and no longer imports Express `Request`.
- Verified:
  - `npx vitest run tests/vm-operate-service.test.ts tests/vm-operation-execution-service.test.ts tests/vm-operation-policy.test.ts` (`3` files, `13` tests)
  - `npm run typecheck`
...
### 2026-05-26 SuperAdmin Facade Request Boundary Slice

- Added `tests/super-admin-service.test.ts`.
- Moved SuperAdmin token/body extraction out of `SuperAdminService` and into `SuperAdminController`.
- `SuperAdminService` now exposes DTO-style methods for role change, CRP assignment, user list, and admin list calls.
- Controller response shapes remain unchanged; SuperAdmin route methods still send the same `resp` payload with the returned status code.
- `SuperAdminService.ts` is now about `62` lines and no longer imports Express `Request`.
- Verified:
  - `npx vitest run tests/super-admin-service.test.ts tests/super-admin-request-adapter-service.test.ts tests/super-admin-user-management-service.test.ts tests/super-admin-user-mutation-policy.test.ts` (`4` files, `13` tests)
  - `npm run typecheck`
...
### 2026-05-26 Course Structure Facade Request Boundary Slice

- Added `tests/class-service.test.ts`.
- Added `tests/chapter-service.test.ts`.
- Moved class/chapter token/body/params extraction out of `ClassService` and `ChapterService` into their controllers.
- `ClassService` and `ChapterService` now expose DTO-style methods and delegate class/chapter route mapping to `CourseStructureRequestAdapterService`.
- Controller response shapes remain unchanged; class/chapter route methods still send the same `resp` payload with the returned status code.
- `ClassService.ts` is now about `38` lines and `ChapterService.ts` is now about `48` lines; neither imports Express `Request`.
- Verified:
  - `npx vitest run tests/class-service.test.ts tests/chapter-service.test.ts tests/course-structure-request-adapter-service.test.ts tests/class-management-service.test.ts tests/chapter-management-service.test.ts tests/class-content-policy.test.ts tests/chapter-content-policy.test.ts` (`7` files, `31` tests)
  - `npm run typecheck`
...
### 2026-05-26 Auth Facade Request Boundary Slice

- Added `tests/auth-service.test.ts`.
- Moved verify/logout token extraction out of `AuthService` and into `AuthController`.
- Moved forgot-password request method/body/authorization extraction out of `AuthService` and into `AuthController`.
- `AuthService` now exposes DTO-style methods for register, login, verify, logout, and forgot-password flows.
- Controller response shapes remain unchanged; Auth route methods still send the same `resp` payload with the returned status code.
- `AuthService.ts` is now about `99` lines and no longer imports Express `Request`.
- Verified:
  - `npx vitest run tests/auth-service.test.ts tests/auth-session-service.test.ts tests/auth-forgot-password-service.test.ts tests/auth-login-service.test.ts tests/auth-registration-service.test.ts tests/auth-token-policy.test.ts` (`6` files, `30` tests)
  - `npm run typecheck`
  - `npm test` (`187` files, `924` tests)
  - `npm run build`
  - `npm audit --audit-level=moderate` (`0` vulnerabilities)
