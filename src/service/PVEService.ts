...
export class PVEService extends Service {
    ...
    public async getDatacenterStatus(Request: Request): Promise<resp<any>> {
        try {
            ...
            // 取得所有節點狀態
            const nodesResp: PVEResp = await callWithUnauthorized('GET', pve_api.nodes, undefined, {
                headers: {
                    'Authorization': `PVEAPIToken=${PVE_API_SUPERADMINMODE_TOKEN}`
                }
            });
            ...
            for (const node of nodes as PVE_NodeStatus[]) {
                const cpu_percent: number = node.maxcpu > 0 ? Math.round(node.cpu * 100) : 0;
                const memory_percent: number = node.maxmem > 0 ? Math.round((node.mem / node.maxmem) * 100) : 0;
                ...
                if (online_status && pve_api.nodes_storage) {
                    try {
                        const nodeStorageResp: PVEResp = await callWithUnauthorized('GET', pve_api.nodes_storage(node.node), undefined, {
                            headers: { 'Authorization': `PVEAPIToken=${PVE_API_SUPERADMINMODE_TOKEN}` }
                        });
                        ...
                        console.error(`Error fetching storage for node ${node.node}:`, e);
                    }
                }
                // 解析 uptime 秒數為天、時、分、秒
            ...
            console.error("Error in getDatacenterStatus:", error);
            return createResponse(500, "Internal Server Error");
        }
    }
}